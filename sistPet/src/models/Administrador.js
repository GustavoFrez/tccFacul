...
const administradorValidation = new Validation('administrador')
...
class Administrador{
    ...
    async insertUser(dataUser){
        ...
        // VALIDAR LOGIN E SENHA
            ...
            let invalidLogin = await administradorValidation.validarLogin(dataUser.login)
            if(invalidLogin) return invalidLogin
            
            let invalidPassword = await administradorValidation.validarSenha(dataUser.senha)
    ...
    async findByLogin(login){
        try{
            ...
            if(user.length > 0) return {status:200, result:user[0]} 
            else return {status:404, result:{erro:`O login ${login} não corresponde a nenhum administrador`}}
        }catch(err){
            console.log(err)
    ...
    async updatePassword(token,newPassword){
        try{
        // VALIDACAO
            let validToken = await administradorValidation.validarToken(token)
            if(validToken.status == 404) return validToken
            let validPassword = await administradorValidation.validarSenha(newPassword)
    ...
    async login(login,senha){
        try{
            if(login == "admin" && senha == "admin") return {status:200, result:{Ok:`Usuário logado com sucesso`,user: "admin"}}
            else {
...
module.exports = new Administrador('administrador','administrador')